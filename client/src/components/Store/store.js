...
class Store extends Component {
    ...
    handleMenuAdd = (e) => {
       ...
       resultdiv.appendChild(countspan);
        ...
        for(let i=0; i<this.state.order.length; ++i) {
            oprice += this.state.price[this.state.order[i]["menu"]] * Number(this.state.order[i]["count"]);
        }
    ...
    handleSubmit = (e) => {
        ...
        const container = document.createElement("div");
            ...
            for(let i=0; i<activeTable.length; ++i) {
                if( activeTable[i].length === 10 && this.state.tmptable == activeTable[i].substr(-1)) {
                    tables[i].innerText = "";
    ...
    componentDidMount() {
        ...
        const orderTable = activeTable.filter((item) => {
            for(let i=0; i<nowTable.length; ++i) if(item == nowTable[i]) return false;
            return true;
        });
                ...
                <div id="myModal" className="modal">
                  <div className="modal-content">
                    ...
                    <form className="order__modal">
                        <div className="add">
                            <p>테이블 선택</p>
                            <select name="selectTable" className="selectTable" onChange={this.handleTableChange}></select>
                        </div>
                        <div className="add">
                            <p>메뉴 선택</p>
                            <select name="selectTable" className="selectMenu" onChange={this.handleMenuChange}>
                                <option>떡볶이</option>
                                <option>순대</option>
                                <option>어묵</option>
                                <option>튀김</option>
                            </select>
                            <input type="text" className="selectCount" placeholder="수량 입력" onChange={this.handleCountChange}></input>
                            <button className="menu--add" onClick={this.handleMenuAdd}>+</button>
                        </div>
                    ...
                    <div className="map__container">
                        ...
                        <div className="hall--space">
                            <Link to="pos/1"><div className="hall--table">테이블 번호 : 1</div></Link>
                            {/* <div className="hall--table">테이블 번호 : 1</div>  */}
                            {/* //here */}
                            <Link to="pos/2"><div className="hall--table --in">2</div></Link>
                            <div className="hall--table">테이블 번호 : 3</div>
                            <Link to="pos/4"><div className="hall--table --in">4</div></Link>
                            <Link to="pos/5"><div className="hall--table">테이블 번호 : 5</div></Link>
                            <Link to="pos/6"><div className="hall--table">테이블 번호 : 6</div></Link>
                        </div>
                    </div>
